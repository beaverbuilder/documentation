...
&nbsp;  |  Columns  |  **(Columns** layout only)<br/> Maximum number of columns in each column group. Readjusts automatically as screen size grows smaller. Clicking the icon next to this field shows the responsive posts display for large, medium, and small devices.
